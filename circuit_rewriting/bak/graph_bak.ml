...
let unify : G.t -> bexp -> G.vertex -> ((bexp, bexp) BatMap.t) list = fun graph -> fun old -> fun tgt_node ->
  ...
  fun subst_list -> fun old -> fun tgt_node ->
    ...
    | (VAR x, _) -> let tgt_bexp = get_bexp_of_node graph tgt_node in BatMap.add (VAR x) tgt_bexp subst
    | (CONST x, _) -> let tgt_bexp = get_bexp_of_node graph tgt_node in BatMap.add (CONST x) tgt_bexp subst
    | (_, Node.VAR x) -> 
      if (snd tgt_node = 0) then 
        BatMap.add old (VAR x) subst
      else
        let pred = G.pred graph tgt_node in
        unify_helper subst old (List.nth pred 0)
    | (_, Node.CONST x) -> BatMap.add old (CONST x) subst
    | (AND (b1, b2), Node.AND)  
    | (OR (b1, b2), Node.OR)  
    | (XOR (b1, b2), Node.XOR) -> 
      let pred = G.pred graph tgt_node in
    ...
    | (NOT b1, Node.AND) -> 
      ...
      let subst_with_b1 = unify_helper subst b1 tgt_b1 in
      subst_with_b1
    | _ -> let tgt_bexp = get_bexp_of_node graph tgt_node in BatMap.add old tgt_bexp subst
  in
