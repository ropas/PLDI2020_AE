...
let rec is_moveup_op : (bexp) -> bool = fun (oldb) ->
  match oldb with
  | NULL -> raise (Error "null used")
  | CONST b -> false
  | VAR x -> false
  | AND (b1, b2) -> (is_moveup_op (b1)) || (is_moveup_op (b2))
  | OR (b1, b2) -> (is_moveup_op (b1)) || (is_moveup_op (b2))
  | XOR (b1, b2) -> true
  | NOT b1 -> false
...
let graph_opt_by_case : G.t -> vdmap -> G.vertex -> (bexp * bexp) -> (G.t * vdmap * bool) = fun graph -> fun vdmap -> fun tgt_node -> fun (old_bexp, new_bexp) ->
  let ilist = get_graph_ilist graph in
  ...
  if(not is_success) then
    ...
    if(not (is_equal_bexp sub_old tgt_bexp)) then
      ...
      if(valid_sub_new_list = []) then
        ...
        let after_depth = get_mult_depth_bexp_with_vdmap new_vdmap sub_new in
        let is_moveup = is_moveup_op (old_bexp) in
        if ((after_depth > before_depth)) then
          (graph, vdmap, false)
        else if((after_depth = before_depth) && (not is_moveup)) then
          (graph, vdmap, false)
        else
          (*let new_vdmap = update_vdmap graph_with_new_tgt vdmap [(lv, sub_new)] in*)
          (*let new_vdmap = update_vdmap graph_with_new_tgt vdmap (List.map (fun x -> (Node.VAR x, 1)) (List.map fst single_eqn)) in*)
          let is_same = true in
(*          if(is_moveup) then
            if(not is_same) then 
...
let node_opt_by_case_list : G.t -> vdmap -> G.vertex -> (bexp * bexp) list -> (G.t * vdmap * bool) = fun graph -> fun vdmap -> fun tgt_node -> fun case_list ->
  ...
  List.fold_left 
    (fun (acc_graph, acc_vdmap, acc_success) -> fun case -> 
       let (new_graph, new_vdmap, success) = graph_opt_by_case acc_graph acc_vdmap tgt_node case in
...
let case_reduce : (bexp * bexp) list -> (bexp * bexp) list = fun case_list ->
  let rec tail_helper : (bexp * bexp) list -> (bexp * bexp) list -> (bexp * bexp) list = fun case_list -> fun acc -> 
    ...
    | hd::tl -> 
      let (old1, new1) = hd in
      let (hd_required, reduced_tl) = 
        List.fold_left
          (fun (acc_required, acc_tl) -> fun (old2, new2) ->
             let old1_size = get_bexp_size old1 in
             let old2_size = get_bexp_size old2 in
             let case1 = if (old1_size > old2_size) then (old2, new2) else (old1, new1) in
             let case2 = if (old1_size > old2_size) then (old1, new1) else (old2, new2) in
             let reducable = case_reducable_commutative case1 case2 in
             let new_required = 
               if(not acc_required) then 
                 false
               else if (not reducable) then 
                 true 
               else if (old1_size > old2_size) then 
                 false
               else
                 true
             in
             let new_tl =
               if(not reducable) then
                 (old2, new2)::acc_tl
               else if (old1_size > old2_size) then
                 (old2, new2)::acc_tl
               else
          ...
          tl
      in
      if(hd_required) then tail_helper reduced_tl (hd::acc) else tail_helper reduced_tl acc
  in
  tail_helper case_list []

let graph_opt_by_case_file : float -> G.t -> string -> string -> G.t = fun time_limit -> fun graph -> fun graph_filename -> fun case_filename ->
  let start_time = Unix.gettimeofday() in
  let _ = tmpvar_index := 0 in
  ...
  let graph_opt_once : G.t -> vdmap -> (var list) -> (G.t * vdmap * bool) = fun graph -> fun vdmap -> fun todo ->
    List.fold_left
      (fun (acc_graph, acc_vdmap, acc_success) -> fun tgt_var ->
        ...
        let (new_graph, new_vdmap, success) = node_opt_by_case_list acc_graph acc_vdmap tgt_node com_case_list in
        (new_graph, new_vdmap, (success || acc_success))
      )
      (graph, vdmap, false)
      todo
  ...
  *)
  let rec opt_iter : (G.t * vdmap * bool) -> int -> var list -> (G.t * vdmap * bool) = fun (acc_graph, acc_vdmap, acc_success) -> fun fail_num -> fun todo ->
    if (todo = []) then
      ...
      let new_cripath = if(fail_num mod 2 = 1) then new_cripath else List.rev new_cripath in
      let _ = print_endline("selected cripath : "); Pp.print_varlist new_cripath; print_newline() in
      let now_time = Unix.gettimeofday() in
      let _ = print_string("now time : "); print_float now_time in
      if((now_time -. start_time) > time_limit) then
