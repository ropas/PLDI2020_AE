...
let print_subst : subst -> unit = fun subst ->
  ...
  let _ = List.iter (fun (old_cbexp, new_cbexp) -> (print_cbexp old_cbexp); print_string("  -->  "); print_cbexp new_cbexp; print_newline()) subst in
  print_endline("---------------------")
...
let rec mem_subst : cbexp -> subst -> bool = fun key -> fun subst ->
  match subst with
  | [] -> false 
  | (b1, b2)::tl -> if(is_equal_cbexp b1 key) then true else mem_subst key tl

let rec find_subst : cbexp -> subst -> cbexp = fun key -> fun subst ->
  match subst with
  | [] -> raise (Error "Key Not Found")
  | (b1, b2)::tl -> if(is_equal_cbexp b1 key) then b2 else find_subst key tl
...
let rec substitute_cbexp : subst -> cbexp -> cbexp = fun subst -> fun cbexp ->
  ...
  let rec subst_once : cbexp -> cbexp = fun cbexp ->
    if(mem_subst cbexp subst) then 
      find_subst cbexp subst 
    else
      match cbexp with
      | CONST _ | CVAR _ -> cbexp
      | VAR _ -> if (mem_subst cbexp subst) then find_subst cbexp subst else cbexp
      | AND (b1, b2) -> 
...
let unify : G.t -> cbexp -> G.vertex -> (subst * bool * bexp) = fun graph -> fun old -> fun tgt_node ->
  ...
  let rec unify_helper : (subst * bool) -> cbexp -> G.vertex -> (subst * bool * bexp) = fun (subst, current_state) -> fun old -> fun tgt_node ->
    if(not current_state) then
      ...
      | (VAR x, Node.CONST b) -> ((VAR x, CONST b)::subst, true, CONST b)
      | (VAR x1, Node.VAR x2) -> ((VAR x1, CVAR x2)::subst, true, VAR x2)
      | (VAR x, _) -> 
        let tgt_bexp = get_bexp_of_node graph tgt_node in 
	let tgt_cbexp = bexp2cbexp tgt_bexp in
	((VAR x, tgt_cbexp)::subst, true, tgt_bexp)
      | (AND (b1, b2), Node.AND)  
      | (OR (b1, b2), Node.OR)  
      | (XOR (b1, b2), Node.XOR) -> 
      ...
      | (_, Node.VAR x) -> 
	    ...
	    if(contain_const old) then
	      (empty_subst, false, false_bexp)
	    else
              ((old, CVAR x)::subst, true, VAR x)
          else
            let pred = G.pred graph tgt_node in
	    ...
	    (*(match (old, (fst pred_node)) with
	    | (AND (b1, b2), Node.AND) 
	    | (AND (b1, b2), Node.VAR _) 
	    | (AND (b1, b2), Node.CONST _) 
	    | (OR (b1, b2), Node.OR)
	    | (OR (b1, b2), Node.VAR _)
	    | (OR (b1, b2), Node.CONST _)
	    | (XOR (b1, b2), Node.XOR) 
	    | (XOR (b1, b2), Node.VAR _)
	    | (XOR (b1, b2), Node.CONST _) -> unify_helper (subst, current_state) old pred_node
	    | (NOT b1, Node.NOT) 
	    | (NOT b1, Node.VAR _) 
	    | (NOT b1, Node.CONST _) -> unify_helper (subst, current_state) old pred_node
	    | _ -> ((old, CVAR x)::subst, true, VAR x)
	    )*)
      | (_, Node.CONST b) -> 
        if(contain_const old) then (empty_subst, false, false_bexp) else ((old, CONST b)::subst, true, CONST b)
      | _ ->
	if(contain_const old) then 
	  (empty_subst, false, false_bexp) 
	else 
          let tgt_bexp = get_bexp_of_node graph tgt_node in 
	  let tgt_cbexp = bexp2cbexp tgt_bexp in
          ((old, tgt_cbexp)::subst, true, tgt_bexp)
  in
